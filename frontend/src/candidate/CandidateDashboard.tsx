...
type Application = { id: number; status: string; currentStage: Stage; submittedAt?: string; updatedAt?: string; jobOfferId: number; jobTitle: string; department?: string; location?: string; formScore?: number; formCandidateComment?: string; formDecision?: string; formEvaluatedAt?: string };
...
function ApplicationDetail({ id, token, applications }: { id: number; token: string; applications: Application[] }) {
  ...
  return <section><PageHeader label="Application Tracking" title={app.jobTitle} body={`${nice(app.status)} - Current stage: ${nice(app.currentStage)}`} /><div className="candidate-grid"><Panel title="Recruitment Timeline"><Timeline current={app.currentStage} /></Panel><Panel title="Application Answers">{responses.length ? responses.map(item => <div className="review-row" key={item.fieldId}><strong>{item.fieldLabel}</strong><span>{text(item.textValue || item.numberValue || item.dateValue || item.booleanValue)}</span></div>) : <EmptyState title="No responses" body="Responses will appear after submission." />}</Panel>{app.formEvaluatedAt && <Panel title="Form evaluation"><div className="review-row"><strong>Score</strong><span>{app.formScore}/100</span></div><div className="review-row"><strong>Decision</strong><span>{nice(app.formDecision)}</span></div><div className="review-row"><strong>Feedback</strong><span>{text(app.formCandidateComment)}</span></div></Panel>}</div></section>;
}
