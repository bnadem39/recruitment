...
export function PreviewMode({ draft, stepIndex, device, onDevice, onStep, onClose }: { draft: BuilderDraft; stepIndex: number; device: string; onDevice: (value: string) => void; onStep: (value: number) => void; onClose: () => void }) {
  ...
  return <div className="fb-preview-mode"><header><div><button onClick={onClose}>Back</button><span><b>Preview mode</b><small>Interactive candidate experience</small></span></div><div className="fb-preview-devices"><button className={device === 'desktop' ? 'active' : ''} onClick={() => onDevice('desktop')}>Desktop</button><button className={device === 'tablet' ? 'active' : ''} onClick={() => onDevice('tablet')}>Tablet</button><button className={device === 'mobile' ? 'active' : ''} onClick={() => onDevice('mobile')}>Mobile</button></div><button className="fb-close-preview" onClick={onClose}>Close preview</button></header><main><div className={`fb-preview-frame ${device}`}><div className="fb-preview-form"><div className="fb-preview-brand"><span>BF</span><b>BFRECRUIT</b></div><div className="fb-preview-progress"><span>STEP {stepIndex + 1} OF {draft.steps.length}</span><i><b style={{ width: `${((stepIndex + 1) / draft.steps.length) * 100}%` }}/></i></div><h1>{step.title}</h1><p>{draft.description}</p><div className="fb-preview-fields">{step.elements.map(element => <FormElement key={element.id} element={element} selected={false} preview />)}</div><div className="fb-candidate-actions"><button className="ghost" disabled={stepIndex === 0} onClick={() => onStep(Math.max(0, stepIndex - 1))}>Back</button><button onClick={() => stepIndex < draft.steps.length - 1 ? onStep(stepIndex + 1) : undefined}>{stepIndex === draft.steps.length - 1 ? 'Submit application' : 'Continue'}</button></div></div></div></main></div>;
}
