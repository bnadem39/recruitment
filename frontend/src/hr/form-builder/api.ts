...
type BackendField = { id: number; label: string; fieldType: BuilderElement['fieldType']; required: boolean; placeholder?: string; displayOrder: number; minimumLength?: number; maximumLength?: number };
type BackendCondition = { id: number; sourceFieldId: number; targetFieldId: number; operator: string; expectedValue?: string; action: string };
type BackendOption = { id: number; label: string; value: string; displayOrder: number };
...
function fieldPayload(element: BuilderElement, order: number) {
  return {
    ...
    maximumLength: element.fieldType === 'FILE' ? (element.maxFileSize || null) : (element.maxLength || null),
...
export async function saveDraftToBackend(draft: BuilderDraft, token: string): Promise<BuilderDraft> {
  ...
  const elements = draft.steps.flatMap(step => step.elements);
...
function estimateHeight(fieldType: BuilderElement['fieldType']) {
  if (fieldType === 'FILE') return 180;
  if (fieldType === 'TEXTAREA') return 126;
  return 94;
}

export async function loadFormDraft(formId: number, token: string): Promise<BuilderDraft> {
  const form = await json<SavedForm>(`/api/forms/${formId}`, token);
  const fields = await json<BackendField[]>(`/api/forms/${formId}/fields`, token);
  const conditions = await json<BackendCondition[]>(`/api/forms/${formId}/conditions`, token);
  ...
  for (const field of sortedFields) {
    ...
    const needsOptions = ['RADIO', 'SELECT', 'MULTI_SELECT', 'CHECKBOX'].includes(field.fieldType);
    let options: string[] | undefined;
    if (needsOptions) {
      const opts = await json<BackendOption[]>(`/api/forms/${formId}/fields/${field.id}/options`, token);
      options = opts.sort((a, b) => a.displayOrder - b.displayOrder).map(o => o.label);
    }
    elements.push({
      id: localId,
      backendId: field.id,
      ...
      help: '',
      internalName: field.label.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, ''),
      ...
      options,
      acceptedFormats: isFile ? 'PDF, DOCX' : undefined,
      maxFileSize: isFile ? (field.maximumLength || 10) : undefined,
    });
  ...
  for (const condition of conditions) {
    ...
    const targetLocal = idByBackend.get(condition.targetFieldId);
    const sourceLocal = idByBackend.get(condition.sourceFieldId);
    if (!targetLocal || !sourceLocal) continue;
    const element = elements.find(item => item.id === targetLocal);
    if (!element) continue;
    const operator = (['EQUALS', 'NOT_EQUALS', 'CONTAINS'].includes(condition.operator) ? condition.operator : 'EQUALS') as 'EQUALS' | 'NOT_EQUALS' | 'CONTAINS';
    element.logic = { sourceId: sourceLocal, operator, value: condition.expectedValue || '', join: 'AND' };
  }
  ...
  return {
    ...
    allowDraft: true,
    allowEditing: false,
    steps: [{ id: uid(), title: 'Page 1', eyebrow: 'Page 1', elements }],
  };
}